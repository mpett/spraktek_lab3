 ...
 * (<code>word</code>) should be in the form <i>word_pos</i> for this 
 * WeightingScheme to work properly. 
 * 
 * @author  Martin Hassel
 * @version 2004-may-23
 */
public class MartinsWCWS extends WeightingScheme {
	...
	public float[] applyLeftWeighting(RandomLabel focusLabel, int distance, RandomLabel leftContextLabel) {
		...
		byte weight = 1;
		if(focusLabel.getWord().matches(".*_[jj|nn|pn|pm|vb]$") && leftContextLabel.getWord().matches(".*_[jj|nn|pn|pm|vb]$"))
			weight = 2;
		...
		for(int current = 0; current < negs.length; current++)
		    focusContext[(int)negs[current]] += (float)((-1/distance)*weight);
		...
		for(int current = 0; current < poss.length; current++)
		    focusContext[(int)poss[current]] += (float)((1/distance)*weight);
		return focusContext;
	}
	...
	public float[] applyRightWeighting(RandomLabel focusLabel, int distance, RandomLabel rightContextLabel) {
		float[] focusContext = focusLabel.getContext();
		byte weight = 1;
		if(focusLabel.getWord().matches(".*_[jj|nn|pn|pm|vb]$") && rightContextLabel.getWord().matches(".*_[jj|nn|pn|pm|vb]$"))
			weight = 2;
		// First we update the negative positions (-1:s)
